...
fn expand_struct(
    ...
    if !cont_attrs.api_skip() {
            ...
            Style::Newtype(field) => {
                ...
                quote! {
                    _nexustack::openapi::HttpResponseBuilder::collect_response(
                        ...
                        #description,
                        #deprecated,
                        <#encoder as _nexustack::openapi::HttpContentType>::describe::<#field_ty, _>,
                    )?;
                }
            }
            Style::Unit => {
        ...
        api_generics
            ...
            .predicates
            .push(syn::parse_quote_spanned! {encoder_span=>
                #encoder: _nexustack::openapi::HttpContentType
            });
...
fn expand_enum(
    ...
    if !cont_attrs.api_skip() {
        ...
        let describe_impl = variants.iter().filter(|variant| !variant.attrs.api_skip()).map(|variant| {
            let style = &variant.style;
            let status_code = variant.attrs.status_code().map(|status_code| quote! {_nexustack::__private::axum::http::StatusCode::#status_code }).unwrap_or(status_code.clone());
