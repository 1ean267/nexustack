...
use crate::{
    ...
    openapi::{expand::Parameters, internals::ast::Container},
};
use proc_macro2::TokenStream;
use quote::quote;

pub fn expand_from_into(cont: &Container, ty: &syn::Type) -> TokenStream {
    let ident = &cont.ident;
    let params = Parameters::new(cont);
    let (impl_generics, ty_generics, where_clause) = params.generics.split_for_impl();
    let body = Stmts(describe(ty));
    let examples = examples_type(ty);
...
fn examples_type(ty: &syn::Type) -> TokenStream {
    quote! { <#ty as _nexustack::openapi::Schema>::Examples }
}

fn describe(ty: &syn::Type) -> Fragment {
