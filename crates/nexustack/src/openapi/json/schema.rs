...
use crate::openapi::{
    ...
    json::{
        SchemaCollection,
        error::Error,
        schema_collection::SchemaCollectionResolutionError,
        specification::{
            self, AdditionalProperties, BoxSchemaOrReferenceObject, ReferenceObject, SchemaObject,
...
fn null_schema(specification: specification::Specification) -> SchemaObject {
    match specification {
        specification::Specification::OpenAPI3_0 => {
            schema! {
                r#type: "string".into(),
                nullable: true,
...
impl Transform<SchemaOrReferenceObject> for AdditionalMapElementTransform<'_> {
    ...
    fn transform(self, i: SchemaOrReferenceObject) -> Result<Self::Output, Self::Error> {
        ...
        if let Some(key_pattern) = self.key_pattern
            && self.schema_builder.specification == specification::Specification::OpenAPI3_1
        {
            ...
            VariantTag::InternallyTagged { tag } => {
                ...
                if let SchemaOrReferenceObject::Schema(schema_object) = &subschema
                    && !schema_object
                        ...
                        .is_some_and(|r#type| match r#type {
                            specification::OneOrMany::One(item) => *item == "object",
                            specification::OneOrMany::Many(items) => {
                ...
                if let SchemaOrReferenceObject::Schema(schema_object) = &mut subschema {
                    ...
                    if let Some(examples) = &mut schema_object.examples {
                        match examples {
                            specification::Examples::Vec(values) => {
                            ...
                            specification::Examples::Map(examples) => {
        ...
        match $self.specification {
            specification::Specification::OpenAPI3_0 => {
                ...
                match $min {
                    ...
                    std::ops::Bound::Excluded(min) => {
                        result.minimum = Some(serde_json::Number::from(min));
                        result.exclusive_minimum = Some(specification::EitherUntagged::Left(true));
                    }
                };
                ...
                match $max {
                    ...
                    std::ops::Bound::Excluded(max) => {
                        result.maximum = Some(serde_json::Number::from(max));
                        result.exclusive_maximum = Some(specification::EitherUntagged::Left(true));
                    }
                };
            ...
            specification::Specification::OpenAPI3_1 => {
                ...
                match $min {
                    ...
                    std::ops::Bound::Excluded(min) => {
                        result.exclusive_minimum = Some(specification::EitherUntagged::Right(
                            serde_json::Number::from(min),
                        ));
                    }
                };
                ...
                match $max {
                    ...
                    std::ops::Bound::Excluded(max) => {
                        result.exclusive_maximum = Some(specification::EitherUntagged::Right(
                            serde_json::Number::from(max),
                        ));
                    }
                };
            }
